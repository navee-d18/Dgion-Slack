...
export default function ThreadPanel({
  ...
  const threadReplies = activeMessages.filter(m => m.parentMessageId === activeThreadMessageId);
        ...
        <div className="bg-slate-50/70 border border-slate-150 rounded-xl p-3.5 relative group shadow-sm select-text">
          {parentMessage ? (
              ...
              <div className="flex-1 min-w-0 ml-2.5 font-sans flex flex-col">
                <div className="flex items-baseline gap-1.5 mb-0.5 select-none">
                  <span className="font-bold text-[13px] text-[#1D1C1D] hover:underline cursor-pointer flex items-center gap-1">
                    ...
                    {bookmarks[parentMessage.id] && <Bookmark className="w-3 h-3 text-amber-500 fill-amber-500 shrink-0" />}
                  </span>
            ...
            <div className="flex items-center gap-2.5 text-slate-400 py-1.5 text-xs font-bold font-sans justify-center select-none">
              <span>⚠️</span>
              <span>Original message deleted</span>
            </div>
          )}
                    ...
                    <>
                      ...
                      <div className="flex-1 min-w-0 ml-2.5 font-sans flex flex-col">
                        <div className="flex items-baseline gap-1.5 mb-0.5 select-none">
                          <span className="font-bold text-[13px] text-[#1D1C1D] hover:underline cursor-pointer flex items-center gap-1">
                            {reply.senderName}
                            {bookmarks[reply.id] && <Bookmark className="w-3 h-3 text-amber-500 fill-amber-500 shrink-0" />}
                          </span>
                          <span className="text-[10px] text-[#616061] font-semibold">
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 animate-in fade-in duration-100 font-sans" role="dialog" aria-modal="true">
          ...
          <div className="relative w-full max-w-sm overflow-hidden bg-white rounded-2xl shadow-slack-modal border border-[#E8E8E8] transition-all duration-200 animate-in zoom-in-95">
            ...
            <div className="p-5 space-y-3">
              <div className="p-3 bg-red-50 border border-red-200 text-red-700 text-xs font-semibold rounded-lg flex items-start gap-2.5">
                <span>⚠️</span>
                <span>Are you sure you want to delete this reply? This action is permanent.</span>
              </div>
            </div>
            {/* Actions */}
            <div className="px-5 py-3.5 bg-slate-50 border-t border-[#E8E8E8] flex justify-end gap-2 select-none">
              <button type="button" onClick={() => setDeleteTargetMessageId(null)} className="px-4 py-2 text-xs font-bold border border-slate-300 rounded-lg hover:bg-slate-105 text-slate-700 cursor-pointer" disabled={isDeleting}>Cancel</button>
              <button
                type="button"
                onClick={async () => {
                  setIsDeleting(true);
